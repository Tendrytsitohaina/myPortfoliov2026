...
const logos = [
	...
	{ id: "react-native", icon: ReactNative, label: "React Native", initialX: -148, initialY: -140 },
...
export default function StickyProfile() {
	...
	const profileX = useTransform(scrollY, [0, 500], shouldReduceMotion ? [0, 0] : [0, 55]);
	...
	const contactOpacity = useTransform(scrollY, [0, 600], [0, 1]);
	...
	return (
		<div className="hidden relative rounded-3xl lg:flex flex-col items-center justify-start px-2 pt-15 sm:px-4">
			...
			>
				<motion.div
					style={{ scale: profileScale, y: profileY, x: profileX }}
					className="mr-30 relative flex items-center justify-center rounded-3xl border border-gray-300 bg-gray-50 shadow-xl transition-colors md:h-50 md:w-50 lg:h-96 lg:w-96 dark:border-gray-800 dark:bg-gray-900"
				>
					<div
			...
			>
				...
				<div className="inline-flex items-center justify-center rounded-full bg-indigo-200 px-3 py-1 text-xs font-semibold text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300">
				...
				<div className="mt-4 flex justify-center gap-3 lg:justify-center">
					<a
